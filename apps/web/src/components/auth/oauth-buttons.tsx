...
const microsoftClientId = process.env.NEXT_PUBLIC_MICROSOFT_OAUTH_CLIENT_ID;
...
export default function OAuthButtons({ mode }: OAuthButtonsProps): ReactElement {
  ...
  // F18: Don't render if no OAuth providers configured
  ...
  const handleMicrosoftSignIn = async (): Promise<void> => {
    setError("");
    try {
      // Microsoft Authentication Library (MSAL.js) popup flow
